...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.word_to_pdf_window = ConvertToPDFWindow()
        self.pdf_to_word_window = ConvertToDocWindow()
    ...
    def _open_secondary_window(self):
        ...
        elif self.main_ui.pdf_to_word_button.isChecked():
            self.pdf_to_word_window.show()
        else:
            self.pdf_merger_window.show()


class ConvertToPDFWindow(QDialog, Ui_WordToPDF):
    def __init__(self):
        super().__init__()
...
class ConvertToDocWindow(QDialog, Ui_PDFToWord):
    def __init__(self):
        super().__init__()
        self.converter_ui = Ui_PDFToWord()
        self.converter_ui.setupUi(self)
        self._input_file = None
        self._output_file = None
        self.connect_signals_and_slots()
    ...
    def _save_file_to_location(self):
        ...
        if check:
            ext_file = os.path.splitext(self._output_file)[1]
            self.converter_ui.output_line_edit.setText("{0}.docx".format(self._output_file)
